...
function plugin() {
  return {
    ...
    post(file) {
      const {
        opts
      } = this; // eslint-disable-next-line prefer-const
    ...
    visitor: _traverse.visitors.merge([{
      Program: {
        enter(path, state) {
          ...
          const pragma = state.defaultedOptions.jsxPragma; // We need to re-export Fragment because of
      ...
      ImportDeclaration: {
        exit(path, state) {
          ...
          if (!tagImports.length) return; // if the tagImports are ALL of the imported values then we want
