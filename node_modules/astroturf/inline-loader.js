...
const hash = name => `${_crypto.default.createHash('md4').update(name).digest('hex')}.cache`;
...
module.exports = async function loader(content, _map, meta) {
  ...
  function resolveDependency(interpolation, localStyle, node) {
    ...
    dependencies.push(buildDependency(request).then(module => {
      ...
      const styleReq = getLoaderRequest(module.resource, style.absoluteFilePath, style.identifier); // replace composes first bc we need need to use a different identifier

      localStyle.value = (0, _replaceComposes.default)(localStyle.value, match => match.replace(source, styleReq).replace(imported, style.type === 'stylesheet' ? memberProperty : 'cls2')); // replace selector interpolations
