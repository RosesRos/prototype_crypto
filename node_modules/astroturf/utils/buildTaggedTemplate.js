...
const buildStyleExpression = (id, className) => t.memberExpression(id, t.stringLiteral(className), // remove the `.`
true);

function assertDynamicInterpolationsLocation(expr, location, opts) {
  var _parent$node$tag;

  const parent = expr.findParent(p => p.isTaggedTemplateExpression()); // may be undefined in the `styled.button` case, or plain css prop case
...
function resolveVariants(exprPath, opts, state) {
  ...
  exprPath.traverse({
    TaggedTemplateExpression(innerPath) {
      ...
      const classId = (0, _camelCase.default)(`${opts.style.identifier}-variant-${state.id++}`); // eslint-disable-next-line @typescript-eslint/no-use-before-define
...
function replaceDependencyPlaceholders(depInterpolations, text, dependencyImports, id, opts) {
  text = (0, _replaceComposes.default)(text, (composes, classList, fromPart) => {
    ...
    return Object.entries((0, _groupBy.default)(composed, i => i.source)).reduce((acc, [source, values]) => {
      const classes = (0, _uniq.default)( // We need to to use the class with the styles for composes
...
function buildTemplateImpl(opts, state = {
  ...
  quasi.quasis.forEach((tmplNode, idx) => {
    ...
    if (lastDynamic && text.endsWith(`var(--${lastDynamic.id})`) && ( // eslint-disable-next-line no-cond-assign
    ...
    if (resolvedDep) {
      ...
      depInterpolations.set(ph, { ...resolvedDep,
        expr
      });
      text += ph;
      return;
    ...
    assertDynamicInterpolationsLocation(expr, location, pluginOptions); // custom properties need to start with a letter
...
function buildTaggedTemplate(opts) {
  ...
  if (location !== 'STYLESHEET' && opts.style.absoluteFilePath.endsWith('.css')) {
    try {
