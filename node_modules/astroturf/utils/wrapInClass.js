...
function hoistImports(text) {
  const imports = [];
  let match;
  const rImports = /@(?:import|use|forward).*?(?:$|;)/g; // eslint-disable-next-line no-cond-assign
...
function wrapInClass(text) {
  const [ruleset, imports] = hoistImports(text); // Components need two css classes, the actual style declarations and a hook class.
