...
const isJsxCallExpression = p => {
  const result = p.isCallExpression() && ( // @ts-ignore
  p.get('callee').referencesImport('react/jsx-runtime') || // @ts-ignore
...
function buildCssProp(valuePath, name, options, isJsx = false) {
  ...
  style.importIdentifier = importId.name; // FIXME?
  // @ts-ignore
...
var _default = {
  Program: {
    exit(path, state) {
      ...
      const pragmaDisabled = !state.defaultedOptions.jsxPragma; // For createElement calls we still need to add an import
